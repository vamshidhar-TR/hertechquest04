 ...
 *
 * With an API key: AI (Opus) writes grounded plain-English rationale in one batched call.
...
import { callAI, firstToolInput, getClient } from './ai.js';
...
async function aiExplain(findings: Finding[], verbosity: 'card' | 'full'): Promise<AiExplanation[]> {
  ...
  const system =
    'You are an AI assistant helping a US individual (Form 1040) tax preparer. For each finding, explain in plain ' +
  ...
  return callAI('explanations (/api/explain)', MODELS.explain, async () => {
    ...
    const out = firstToolInput<{ explanations: AiExplanation[] }>(msg);
    if (!out?.explanations) throw new Error('no explanations in response');
    return out.explanations;
  });
}

/** Answer a spoken follow-up about one finding — AI when available, else a grounded template. */
export async function answerFollowup(finding: Finding, question: string): Promise<{ answer: string; citation: Citation; answered_via: 'ai' | 'deterministic' }> {
  ...
  if (aiAvailable()) {
    ...
    try {
      if (client) {
        const text = await callAI('voice follow-up (/api/ask)', MODELS.explain, async () => {
          const msg = await client.messages.create({
            model: MODELS.explain,
            max_tokens: 400,
            ...temperatureParam(),
            system:
              'You are an AI assistant helping a US tax preparer by voice. Answer the spoken follow-up about this single ' +
        ...
        if (text) return { answer: text, citation, answered_via: 'ai' };
      }
    } catch {
      /* fall through */
    }
...
export async function explainFindings(
  ...
  if (aiAvailable()) {
    try {
      const ex = await aiExplain(findings, verbosity);
      const map = new Map(ex.map((e) => [e.finding_id, e]));
      return {
        explanations: findings.map((f) => {
          const hit = map.get(f.finding_id);
          // Always hand the UI a clickable source: use AI's citation only if it has a URL,
          // otherwise fall back to our deterministic IRS link (AI usually emits a label, no URL).
